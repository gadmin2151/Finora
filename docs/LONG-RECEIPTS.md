# Live vertical receipt scanner · Android 1.8.1
...
- The camera requests a 4K analysis stream with fallback to a supported size. Only the paper window is copied from the luminance plane at each capture, avoiding full-frame RGBA conversion. Rendering, review and draft import preserve 8-bit grayscale precision rather than RGB_565 quantization. Output is bounded to 1,920 pixels in width, 14,000 pixels in height and 160 appended sections; very long images are proportionally reduced to a device-heap-based pixel budget, capped at 16 megapixels. Capture pauses in the background. Temporary sections remain in private app cache and are cleaned when leaving; the chosen result is copied to the existing draft store.
