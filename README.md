...
Нативный клиент для Android 8+: HTTPS-вход на свой сервер, выбор организации, QR электронных чеков, съёмка одного чека несколькими фотографиями, сохранение черновика, история и месячная статистика. После распознавания приложение показывает магазин, дату, позиции с количеством и ценами, категории и итог. Расход создаётся только после подтверждения. Исходники, подписанный APK и проверки — в [android/README.md](android/README.md). Дополнительный AI на телефоне не требуется.
...
Ссылки MEV/SIFT с идентификатором чека приводятся к единому адресу. Для остальных публичных HTTPS-ссылок сервер следует перенаправлениям, открывает страницу в изолированном Chromium и сохраняет полный снимок. OCR и настроенная AI-модель выделяют товары; снимок остаётся в карточке для сверки. Ссылки на локальную сеть и служебные адреса запрещены, каждый сетевой переход проверяется, размеры страницы и время загрузки ограничены. Страницы, требующие авторизации, CAPTCHA, отправки форм, а также PDF могут потребовать фотографии бумажного чека.

Android отправляет `review_required=true` для фото и QR. Такие чеки, а также любые чеки с универсальных веб-страниц, остаются черновиками независимо от настройки автоматического учёта. Кнопка «Всё верно · подтвердить» создаёт расход после проверки суммы строк и выбора счёта в валюте чека. Повторное подтверждение не создаёт дубль. Если данные неполные или суммы расходятся, администратор исправляет их в веб-версии.

`POST /api/receipts/{id}/accept` принимает только `version` и `account_id`; цены и позиции берутся с сервера. Подтвердить может автор или администратор той же организации. Миграция `f30a8210de91` добавляет признак обязательной проверки и автора, сохраняя существующие чеки и пользователей. Выполните обычное обновление через `scripts/deploy.py` перед установкой Android 1.1; скрипт создаёт резервную копию перед миграцией. Старые клиенты и административный `/confirm` совместимы.
